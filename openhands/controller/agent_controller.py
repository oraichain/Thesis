...
class AgentController:
    ...
    async def _step_with_exception_handling(self):
        try:
            if shared_config.enable_test_refund_credit and self.space_section_id:
                raise LLMContextWindowExceedError('Conversation history longer than')
            await self._step()
    ...
    async def _extract_and_save_final_result(
        ...
        Uses the utility function for consistent result extraction across components.
        """
        try:
