...
def sync_generate_conversation_title(
    ...
    try:
        ...
        messages = [
            {
                'role': 'system',
                'content': 'You are a helpful assistant that generates concise, descriptive titles for conversations with Thesis. Thesis is an AI research agent that helps users gather information, analyse data, and deliver clear, well-supported insights. Given a user message (which may be truncated), generate a concise, descriptive title for the conversation. Return only the title, with no additional text, quotes, or explanations.',
            },
            {
                'role': 'user',
